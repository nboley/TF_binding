...
def bin_energies(energies, min_energy, max_energy, n_bins=1000):
    ...
    x = np.linspace(min_energy, min_energy+step_size*len(hist_est), len(hist_est));    
...
def cmp_to_brute():
    ...
    x, part_fn = est_partition_fn(ref_energy, ddg_array, 256)
    min_cdf = 1 - (1 - part_fn.cumsum())**2
    min_pdf = np.array((min_cdf[1:] - min_cdf[:-1]).tolist() + [0.0,])
    x2, part_fn_brute = est_partition_fn_by_brute_force(motif, 256)
